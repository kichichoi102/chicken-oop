...
def test_can_init(animal_habitat_director):
    assert isinstance(animal_habitat_director.builder, ConcreteChickenCoopBuilder)

def test_can_build_habitat(animal_habitat_director):
    animal_habitat_director.build_habitat(10, "Wood")
    assert (animal_habitat_director.builder.chicken_coop.capacity == 10) \
        and (animal_habitat_director.builder.chicken_coop.material == "Wood")
...
def test_can_add_animal(animal_habitat_director, concrete_chicken_coop_builder, chicken_builder):
    ...
    chicken = animal_habitat_director.builder.chicken_coop.chickens[0]
    assert (chicken.species == "Chicken" and chicken.name == "Name" and chicken.weight == 10)
